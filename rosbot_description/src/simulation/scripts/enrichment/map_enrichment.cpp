...
int main(int argc, char * argv[]) {
    ...
    ros::Publisher publisher_signs = nh.advertise<nav_msgs::OccupancyGrid>("/signs", 1);

    nav_msgs::GetMap srv;
    client.call(srv);
    nav_msgs::OccupancyGrid *occupancyGridInput = &srv.response.map;
    ROS_INFO("Static map size: %i", occupancyGridInput->data.size());
    ROS_INFO("width: %i", occupancyGridInput->info.width);
    ROS_INFO("height: %i", occupancyGridInput->info.height);

    grid_map::GridMap *map = new grid_map::GridMap({"static_map_layer", "signs_layer"});
    grid_map::GridMapRosConverter::fromOccupancyGrid(*occupancyGridInput, "static_map_layer", *map);
    map->setFrameId(occupancyGridInput->header.frame_id);
