...
public:
    explicit MapEnrichment(ros::NodeHandle& nh) {
        client_map_server = nh.serviceClient<nav_msgs::GetMap>(str(constants::REQ_ORIGINAL_MAP));
        srv_add_sign = nh.advertiseService(str(constants::ADV_ENRICH_AUGMENTED_MAP), &MapEnrichment::addSignOnMap,this);
        srv_remove_sign = nh.advertiseService(str(constants::ADV_IMPOVERISH_AUGMENTED_MAP), &MapEnrichment::removeSignFromMap, this);
        pub_map = nh.advertise<nav_msgs::OccupancyGrid>(str(constants::PUB_AUGMENTED_MAP), 1, true);
...
int main(int argc, char * argv[]) {
    ...
    ros::NodeHandle nh("~");
    MapEnrichment mapEnrichment(nh);
    ros::spin();
    return 0;
}
