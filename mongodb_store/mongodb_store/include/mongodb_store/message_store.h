        ...
        **/
        ...
        std::string insert(const MsgType &_msg,
            ...
            if (_wait) {
                ...
                if (!_meta.isEmpty()) {
                    srv.request.meta.pairs.push_back(
                            makePair(mongodb_store_msgs::MongoQueryMsgRequest::JSON_QUERY, _meta.jsonString()));
                }
        ...
        template<typename MsgType>
        bool queryID(const std::string &_id, std::vector<boost::shared_ptr<MsgType> > &_messages) {
        ...
        int _limit = 0) {
            ...
            if (!_message_query.isEmpty()) {
                msg.request.message_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQueryMsgRequest::JSON_QUERY, _message_query.jsonString()));
            }

            //if there's no meta then no copying is necessary
            if (!_meta_query.isEmpty()) {
                msg.request.meta_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQueryMsgRequest::JSON_QUERY, _meta_query.jsonString()));
            }
            //if there's no sort message then no copying is necessary
            if (!_sort_query.isEmpty()) {
                msg.request.sort_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQueryMsgRequest::JSON_QUERY, _sort_query.jsonString()));
            }
            ...
            if (m_queryClient.call(msg)) {
                ...
                if (msg.response.messages.size() > 0) {
                    ...
                    for (size_t i = 0; i < msg.response.messages.size(); i++) {
                        ...
                        if (_decode_metas && msg.response.metas[i].pairs[0].first ==
                                             mongodb_store_msgs::MongoQueryMsgRequest::JSON_QUERY) {
                            _messages.push_back(std::make_pair(deserialise_message<MsgType>(msg.response.messages[i]),
                                                               mongo::fromjson(msg.response.metas[i].pairs[0].second)));
                        } else {
                            if (_decode_metas) {
        ...
        int _limit = 0) {
            ...
            if (!_message_query.isEmpty()) {
                msg.request.message_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQuerywithProjectionMsgRequest::JSON_QUERY,
                                 _message_query.jsonString()));
            }
            ...
            if (!_meta_query.isEmpty()) {
                msg.request.meta_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQuerywithProjectionMsgRequest::JSON_QUERY,
                                 _meta_query.jsonString()));
            }
            //if there's no sort message then no copying is necessary
            if (!_sort_query.isEmpty()) {
                msg.request.sort_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQuerywithProjectionMsgRequest::JSON_QUERY,
                                 _sort_query.jsonString()));
            }
            //if there's no projection message then no copying is necessary
            if (!_projection_query.isEmpty()) {
                msg.request.projection_query.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQuerywithProjectionMsgRequest::JSON_QUERY,
                                 _projection_query.jsonString()));
            }
            ...
            if (m_querywithProjectionClient.call(msg)) {
                ...
                if (msg.response.messages.size() > 0) {
                    ...
                    for (size_t i = 0; i < msg.response.messages.size(); i++) {
                        ...
                        if (_decode_metas && msg.response.metas[i].pairs[0].first ==
                                             mongodb_store_msgs::MongoQuerywithProjectionMsgRequest::JSON_QUERY) {
                            _messages.push_back(std::make_pair(deserialise_message<MsgType>(msg.response.messages[i]),
                                                               mongo::fromjson(msg.response.metas[i].pairs[0].second)));
                        } else {
                            if (_decode_metas) {
        ...
        bool queryWithProjection(std::vector<boost::shared_ptr<MsgType> > &_messages,
            ...
            std::vector<std::pair<boost::shared_ptr<MsgType>, mongo::BSONObj> > msg_and_metas;
            bool result = queryWithProjection(msg_and_metas, _message_query, _meta_query, _sort_query,
                                              _projection_query, _find_one, false, _limit);

            for (typename std::vector<std::pair<boost::shared_ptr<MsgType>, mongo::BSONObj> >::iterator i = msg_and_metas.begin();
                    i != msg_and_metas.end(); ++i) {
        ...
        bool update(const MsgType &_msg,
            ...
            //if there's no meta then no copying is necessary
            if (!_meta.isEmpty()) {
                msg.request.meta.pairs.push_back(
                        makePair(mongodb_store_msgs::MongoQueryMsgRequest::JSON_QUERY, _meta.jsonString()));
            }
